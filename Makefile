...
DOCKER      	?= docker
IMAGE_NAME  	:= $(REGISTRY)/routeros2rest-bridge
...
build-local: gen
	...
	go fmt ./...
	go mod tidy
	GOOS=$(OS) GOARCH=$(ARCH) CGO_ENABLED=0 go build -ldflags "$(LDFLAGS)" -o dist/app pkg/cmd/main.go

lint:
	pre-commit run --all-files

test:
	go test $$(go list ./... | grep -v /api | grep -v /types) -coverprofile=coverage.out
